...
inline Promise::Resolver Promise::Resolver::New(napi_env env) {
  return Promise::Resolver(env);
}

inline Promise::Resolver::Resolver(napi_env env) : _env(env) {
  napi_status status = napi_create_promise(_env, &_deferred, &_promise);
  NAPI_THROW_IF_FAILED(_env, status);
}

inline Promise Promise::Resolver::Promise() const {
  return Napi::Promise(_env, _promise);
}

inline void Promise::Resolver::Resolve(napi_value value) const {
  napi_status status = napi_resolve_deferred(_env, _deferred, value);
  NAPI_THROW_IF_FAILED(_env, status);
}

inline void Promise::Resolver::Reject(napi_value value) const {
