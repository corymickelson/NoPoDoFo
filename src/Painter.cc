...
#include "Painter.h"
#include "Document.h"
Painter::Painter(const Napi::CallbackInfo &info) : ObjectWrap(info)
{
}
void
...
//  height = info[4].As<Number>();
  ...
  _painter->DrawImage(
    0.0, pageSize.GetHeight() - image.GetHeight(), &image);
}

Napi::Value
Painter::GetPrecision(const CallbackInfo &info)
{
  return Napi::Number::New(info.Env(), static_cast<double>(_painter->GetPrecision()));
}
