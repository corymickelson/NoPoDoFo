...
void Configure::Initialize(Napi::Env &env, Napi::Object &target) {
  HandleScope scope(env);
  const char *klass = "Configure";
  auto ctor = DefineClass(
      env, klass,
      {InstanceAccessor("enableDebugLogging", &Configure::GetDebugLogging,
                        &Configure::EnableDebugLogging)});
  Constructor = Persistent(ctor);
  Constructor.SuppressDestruct();
  target.Set(klass, ctor);
}
Configure::Configure(const Napi::CallbackInfo &info) : ObjectWrap(info) {
  DbgLog = spdlog::get("DbgLog");
}
void Configure::EnableDebugLogging(const CallbackInfo &info,
                                   const JsValue &value) {
...
JsValue Configure::GetDebugLogging(const Napi::CallbackInfo &info) {
  bool enabled = false;
