...
public:
  ...
  void
  SetPage(const CallbackInfo &);
  void
  FinishPage(const CallbackInfo &);
  void
  DrawText(const CallbackInfo &);
  void
  DrawImage(const CallbackInfo &);
  Napi::Value
  GetPrecision(const CallbackInfo &);
  PoDoFo::PdfMemDocument *
  GetDocument() { return _document; }
  PoDoFo::PdfPainter *
  GetPainter() { return _painter; }
private:
