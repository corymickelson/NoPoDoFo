	...
	try {
						...
						if (permissions.Get(i).IsString()) {
							...
							if (permission == "Copy")
								...
								msg << "Unknown permission parameter: " << permission
								    ...
								    << "[Copy, Print, Edit, EditNotes, FillAndSign, Accessible, DocAssembly, HighPrint]"
								    << endl;
		...
		if (encryption.Has("algorithm")) {
			...
			if (algoProp.IsString()) {
				...
				if (algo == "rc4v1") algoParameter = 1;
				else if (algo == "rc4v2") algoParameter = 2;
				else if (algo == "aesv2") algoParameter = 4;
				else if (algo == "aesv3") algoParameter = 8;
				else {
					stringstream msg;
					msg << "Unknown permission parameter: " << algo
					    << ". Permission must be one or more of: [rc4v1, rc4v2, aesv2, aesv3]"
					    << endl;
					throw Napi::Error::New(info.Env(), msg.str());
				}
			}
		...
		if (encryption.Has("keyLength")) {
			...
			if (keyProp.IsNumber()) {
				...
				for (int i = 0; i < 6; ++i) {
					if (keyValues[i] == key) break;
					if (keyValues[i] != key && i == 6) {
						stringstream msg;
						msg << "Unknown permission parameter: " << key
						    << ". Permission must be one or more of: [40, 56, 80, 96, 128, 256]"
						    << endl;
						throw Napi::Error::New(info.Env(), msg.str());
					}
				}
		...
		_document->SetEncrypted(userPwd, ownerPwd, permParameter,
		                        static_cast<PoDoFo::PdfEncrypt::EPdfEncryptAlgorithm>(algoParameter),
		                        static_cast<PoDoFo::PdfEncrypt::EPdfKeyLength>(key));
	}
	catch (PoDoFo::PdfError&err) {
		stringstream msg;
		msg << "PoDoFo::PdfMemDocument::SetEncrypt failed with error: " << err.GetError()
		    << endl;
		throw Napi::Error::New(info.Env(), msg.str());
	}
