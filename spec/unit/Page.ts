...
export class pageSpec {
    ...
    @AsyncTest('Page delete form fields')
    public async pageDeleteFormFields() {
        const page = this.mem.getPage(0)
        const initialFields = page.fieldCount()
        Expect(initialFields).toBeGreaterThan(0)
        page.deleteField(0)
        Expect(page.fieldCount()).toBeLessThan(initialFields)
        await new Promise(resolve => {
            this.mem.write((err, data) => {
                if (err) Expect.fail(err.message)
                new Promise(resolve => {
                    const child = new Document()
                    child.load(data, (e, d) => {
                        if (e) Expect.fail(e.message)
                        Expect(child.getPage(0).fieldCount()).toEqual(initialFields - 1)
                        return resolve()
                    })
                })
                    .then(() => resolve())
